...
function startDownload(event) {
  event.preventDefault(); // منع الرابط من التحميل فوراً

  // إخفاء الزر وعرض شريط التحميل
  const downloadButton = event.target.closest('a');
  downloadButton.style.display = 'none';
  
  const progressBarContainer = downloadButton.nextElementSibling;
  ...
  const interval = setInterval(() => {
    ...
    if (progress >= 100) {
      ...
      // بعد اكتمال التحميل، تحويل المستخدم إلى الرابط
